;;; cacao-test-theme.el --- Based on a image reversal
...
;;  (set-face-attribute 'org-code nil :inherit 'variable-pitch)
;;   (set-face-attribute 'org-block nil :inherit 'variable-pitch)
...
(defun set-buffer-variable-pitch()
  "Enable variable-pitch for the curren buffer, but retain fixed-pitch for specific elements like tables and code blocks."
  (interactive)
  (variable-pitch-mode t)
  ;; Force specific faces to fixed-pitch for proper alignment
  (set-face-attribute 'org-table nil :inherit 'fixed-pitch))
...
(provide-theme 'cacao-test)

;; Local Variables
;; End
;;; cacao-test-theme.el ends here
